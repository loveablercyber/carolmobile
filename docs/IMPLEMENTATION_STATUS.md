...
- **Deploy:** publicado em Production no Vercel no deployment `dpl_FkxrGezchM1kWpEW8k8NcwJjhthp`, com alias `https://carolmobile.vercel.app`.
...
- **Causa identificada:** o `/api/status` do bot externo mostrou o webhook apontando para `carolsol.vercel.app/api/webhooks/baileys/carolsol`, domínio antigo que respondeu `401 Webhook nao autorizado`. A URL correta do PWA ativo é `carolmobile.vercel.app/api/webhooks/baileys/carolsol`, que respondeu `200`.
...
- **Correção no bot externo:** o repositório `loveablercyber/whats` recebeu diagnóstico de inbound/webhook no `/api/status`, com alvo do webhook sem segredo, último inbound, se veio de `fromMe`, tentativa/status/erro do webhook e fallback mais seguro quando `WEBHOOK_URL` aponta para a própria API Render ou para o domínio antigo `carolsol.vercel.app`.
...
- **Validação pós-Render:** após auto-deploy do Render, `/api/whatsapp?resource=panel` retornou `provider.webhook.usingFallback=true` e `target.host=carolmobile.vercel.app`, mantendo `connectionStatus=connected` e `liveStatus=connected`.
