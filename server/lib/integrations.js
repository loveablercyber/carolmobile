...
function hashIndex(value, length) {
  ...
  const hash = createHash('sha256').update(String(value || Date.now())).digest()
...
export function cloudinaryProviders() {
  ...
  })).filter(provider => provider.cloudName && provider.apiKey && provider.apiSecret)
}
...
export function createCloudinarySignature(key) {
  ...
  const signature = createHash('sha1').update(`folder=${folder}&timestamp=${timestamp}${provider.apiSecret}`).digest('hex')
  return { cloudName: provider.cloudName, apiKey: provider.apiKey, timestamp, folder, signature, provider: provider.name }
}
...
export async function sendWhatsApp({ to, text }) {
  if (!truthy(process.env.BAILEYS_ENABLED) || !process.env.BAILEYS_API_URL || !to) return { skipped: true }
  ...
  const instance = instances[index % Math.max(instances.length, 1)] || instances[0] || 'default'
  ...
  const attempts = [
    { url: `${base}/message/sendText/${encodeURIComponent(instance)}`, body: { number, text } },
    { url: `${base}/api/send-message`, body: { instance, phone: number, message: text } }
  ]
  let lastStatus = 0
...
export async function notifyAppointment({ email, phone, clientName, service, date, professional }) {
  const prettyDate = new Date(date).toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo', dateStyle: 'short', timeStyle: 'short' })
  const text = `Olá, ${clientName}! Seu agendamento de ${service} com ${professional} foi reservado para ${prettyDate}. — Carol Sol`
  return Promise.allSettled([
    sendEmail({ to: email, subject: 'Agendamento Carol Sol confirmado', html: `<div style="font-family:Arial,sans-serif;color:#181511"><h1>Seu horário está reservado ✨</h1><p>${text}</p><p>Em caso de dúvida, responda esta mensagem.</p></div>` }),
    sendWhatsApp({ to: phone, text }),
    process.env.ADMIN_NOTIFICATION_EMAIL ? sendEmail({ to: process.env.ADMIN_NOTIFICATION_EMAIL, subject: 'Novo agendamento — Carol Sol', html: `<p>${clientName} reservou ${service} para ${prettyDate} com ${professional}.</p>` }) : Promise.resolve({ skipped: true })
  ])
}
