...
export function ProfessionalProfilePage(){const{refresh}=useAuth();const p=useLoad<any>('/api/portal?resource=profile');const[form,setForm]=useState<any>({});const[saving,setSaving]=useState(false);const[toast,setToast]=useState('');const[uploadingPhoto,setUploadingPhoto]=useState(false);useEffect(()=>{if(p.data)setForm({fullName:p.data.full_name,email:p.data.email,phone:p.data.phone||'',birthDate:p.data.birth_date?.slice(0,10)||'',instagram:p.data.instagram||'',address:p.data.address||{},avatarUrl:p.data.avatar_url||'',bio:p.data.bio||'',specialties:p.data.specialties||[]})},[p.data]);if(p.loading)return <LoadingState/>;const save=async(e:FormEvent)=>{e.preventDefault();setSaving(true);try{await apiFetch('/api/portal?resource=profile',{method:'PATCH',body:JSON.stringify(form)});await Promise.all([p.reload(),refresh()]);setToast('Alterações salvas com sucesso.')}catch(err){console.error('Professional profile error',err);setToast(err instanceof Error?err.message:'Ocorreu um erro ao salvar os dados.')}finally{setSaving(false);setTimeout(()=>setToast(''),2200)}};return <div><Toast show={!!toast} message={toast}/><PageHeader eyebrow="PERFIL PROFISSIONAL" title={p.data?.full_name||'Perfil'} subtitle="Dados, biografia e especialidades persistidos no Neon."/><form onSubmit={save} className="grid gap-5 lg:grid-cols-[300px_1fr]"><aside className="surface p-7 text-center"><Avatar src={form.avatarUrl} name={form.fullName||''} size="lg"/><label className="btn-secondary mt-4 cursor-pointer">{uploadingPhoto?'Enviando…':'Trocar foto'}<input type="file" accept="image/*" className="hidden" disabled={uploadingPhoto} onChange={async(e)=>{const file=e.target.files?.[0];if(!file)return;setUploadingPhoto(true);try{const uploaded=await uploadImage(file,'profile-photo');setForm((f:any)=>({...f,avatarUrl:uploaded.url}));await apiFetch('/api/portal?resource=profile',{method:'PATCH',body:JSON.stringify({...form,avatarUrl:uploaded.url})});setToast('Foto atualizada.');}catch(err){setToast(err instanceof Error?err.message:'Não foi possível enviar a foto.');}finally{setUploadingPhoto(false);setTimeout(()=>setToast(''),2200);}}}/></label><h2 className="mt-4 font-display text-3xl">{form.fullName}</h2><p className="muted mt-2">{p.data?.active?'Profissional ativa':'Profissional inativa'}</p></aside><section className="surface p-6"><div className="grid gap-4 sm:grid-cols-2"><Input label="Nome" value={form.fullName||''} set={v=>setForm({...form,fullName:v})}/><Input label="E-mail" type="email" value={form.email||''} set={v=>setForm({...form,email:v})}/><Input label="Telefone" value={form.phone||''} set={v=>setForm({...form,phone:v})}/><Input label="Instagram" value={form.instagram||''} set={v=>setForm({...form,instagram:v})}/></div><label className="mt-4 block"><span className="mb-2 block text-xs font-bold">Biografia</span><textarea className="field min-h-24 py-3" value={form.bio||''} onChange={e=>setForm({...form,bio:e.target.value})}/></label><label className="mt-4 block"><span className="mb-2 block text-xs font-bold">Especialidades, separadas por vírgula</span><input className="field" value={(form.specialties||[]).join(', ')} onChange={e=>setForm({...form,specialties:e.target.value.split(',').map(x=>x.trim()).filter(Boolean)})}/></label><button disabled={saving} className="btn-primary mt-5"><Save size={15}/>{saving?'Salvando…':'Salvar perfil'}</button></section></form></div>}
